...
import { fetchGroups, groupsSelector, getGroupsFiltered } from "../../store/slices/groups";
...
const GroupsList = () => {
  ...
  const renderGroups = () => {
    if (hasErrors) return <Table.Row><Table.Cell>Não foi possível exibir grupos...</Table.Cell></Table.Row>;
    return filteredGroups.map(group => (
      <Table.Row key={group.id} >
        <Table.Cell>{group.descricao}</Table.Cell>
        <Table.Cell className="has-text-centered">{group.cat_inicio}</Table.Cell>
            ...
            <Table fullwidth bordered hoverable>
              <Table.Head backgroundColor="danger">
                <Table.Row>
                  ...
                  <Table.Heading textColor="white" className="has-text-centered">Cat. Inicio</Table.Heading>
                  <Table.Heading textColor="white" className="has-text-centered">Cat. Fim</Table.Heading>
                  <Table.Heading textColor="white" className="has-text-centered">Ações</Table.Heading>
                </Table.Row>
              </Table.Head>
              <Table.Body>
                {renderGroups()}
